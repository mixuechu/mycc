...
def analyze_conversation(conversation_summary: str, memory_dir: str):
    ...
    Args:
        conversation_summary: 对话摘要（最近几轮对话）
    ...
    # 构造分析 prompt  
    analysis_prompt = f"""你是一个智能记忆系统管理员。分析这次对话，判断需要更新哪些记忆文档。
...
**更新标准**：
1. status.md - 日期不是今天 OR 有重大进展
2. context.md - 今天有重要进展需要记录
3. about-me - 发现核心偏好/能力变化
4. CC Memory - 值得长期记住的对话
...
"""
    ...
    try:
        ...
        return {
            ...
            "update_aboutme": False,
            "archive_cc_memory": False,
            "reasoning": f"分析失败: {str(e)}"
        }

def main():
    if len(sys.argv) < 3:
        print("Usage: analyze_memory.py <conversation_summary> <memory_dir>")
        sys.exit(1)
